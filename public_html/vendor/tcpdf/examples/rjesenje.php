...
//            . "WHERE STATUS LIKE '01%' AND djel.KLINIKA = ".$klinika." ";
    $query = "SELECT *, klinike.NAZIV as klinika, nomrm.NAZIV as RM, nomtit.IDe as TITULA "
            . "FROM djel "
            . "LEFT JOIN godod "
            . "ON djel.ID = godod.DJEL "
...
while($row =  mysqli_fetch_assoc($result)){
    ...
    $pdf->Ln(7);
    $pdf->Cell(180, 5, 'Mostar, 10.4.2017. godine');
    $pdf->Ln(10);
    ...
    $pdf->Cell(5, 5, '1.');
    $pdf->SetFont('dejavusans', 'B', 11);
    $pdf->Cell(65, 5, ' '.$row["TITULA"].'  '.$row["IME"].' ' .$row["PREZIME"].',', 'B', 0, 'C', 0, 0, 1);
    $pdf->SetFont('dejavusans', '', 11);
    $pdf->Cell(115, 5, ' radniku na radnom mjestu '.$rm.' na ', '', 0, '', 0, 0, 1);
    $pdf->Ln(5);
    $klinika = mb_strtolower($row["klinika"], 'UTF-8');
    $klinika = ucfirst($klinika);
    $pdf->Cell(70, 5, ' '.$klinika.' ', 'B', 0, 'J', 0, 0, 1);
    $pdf->SetFont('dejavusans', 'B', 11);
    $pdf->Cell(110, 5, ' utvrđuje se pravo na godišnji odmor za 2017. godinu, ', '', 0, '', 0, 0, 1);
    $pdf->Ln(5);
    $pdf->Cell(73, 5, '    u trajanju od ukupno '.$row["UKUPNO"].' radnih dana, ', '', 0, '', 0, 0, 1);
    $pdf->SetFont('dejavusans', '', 11);
...
//    if($row["STATUS"]==='0102' | $row["STATUS"]==='010201'){
    ...
    $pdf->SetFont('dejavusans', '', 11);
    $pdf->Ln(5);
    $pdf->Cell(180, 5, 'Sukladno članku 48. Zakona o radu ("Službene novine Federacije BiH", broj 26/16)', '', 0, 'J', 0, 0, 1);
    $pdf->Ln(5);
    ...
    $pdf->Cell(180, 5, '2. Radnik će prvi dio odmora koristit od '.date2mysql($row["GODODOD"]).'. do '.date2mysql($row["GODODDO"]).'. godine, a drugi dio u', '', 0, 'J', 0, 0, 1);
    $pdf->Ln(5);
    $pdf->Cell(180, 5, '   dogovoru s poslodavcem.');
    $pdf->Ln(10);
    $pdf->Cell(180, 5, '3. Jedan dan godišnjeg odmora radnik može koristiti kad  želi, uz obavezu da o tome obavijesti', '', 0, 'J', 0, 0, 1);
    $pdf->Ln(5);
    $pdf->Cell(180, 5, '   poslodavca namjanje tri dana prije njegovog korištenja.', '', 0, '', 0, 0, 1);
    $pdf->Ln(10);
    $pdf->Cell(180, 5, '4. Za vrijeme korištenja godišnjeg odmora radnik ima pravo na naknadu plaće u skladu sa', '', 0, 'J', 0, 0, 1);
    $pdf->Ln(5);
    $pdf->Cell(180, 5, '   člankom 52. stav 3. Zakona o radu.', '', 0, '', 0, 0, 1);    
    $pdf->Ln(12);
    $pdf->Cell(75);
    ...
    $pdf->Ln(5);
    $pdf->Cell(180, 5, 'odmora za 2017. godinu, koji je donesen uz prethodnu konsultaciju sa radnicima i ', '', 0, 'J', 0, 0, 1);
    $pdf->Ln(5);
