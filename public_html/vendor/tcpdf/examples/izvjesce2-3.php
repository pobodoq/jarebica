...
//DODAJI KOMENTARE TOPE ZA SVE, ZA DVA DANA CES U OVAJ KOD GLEDAT KO GLUVA GUDA
//dakle, upit za odjele jer po njima ispisujem
...
// set color for background
    $pdf->SetFillColor(255, 255, 127);
    $query = "SELECT IME, PREZIME, INVALID, klinika.NAZIV as klk "
            . "FROM djel "
            . "LEFT JOIN klinika "
            . "ON djel.KLINIKA = klinika.ID "
            . "WHERE klinika LIKE '".$rowy[2]."%' ".$where." ";
//$query = "SELECT IME, PREZIME, BRD, OTAC, JMBG, DATUMR, SSTAZDANI, SSTAZMJ, SSTAZGOD, DZRO, DPRO, nomstatusi.NAZIV AS STATUS, nomtit.IDe AS TITULA, nomrm.NAZIV AS RM, nomzan.NAZIV AS ZANIMANJE, nomskspr.NAZIV AS SKSP, nomss.NAZIV AS SS, kbodjeli.NAZIV AS ODJEL "
//        . "FROM djel "
//        . "LEFT JOIN nomtit "
...
//FIELD(nomzan.ID,'3','4','5'), nomzan.NAZIV DESC  ovo je za order by field... koji field zelis.. odlicno ha...
    ...
    $pdf->Cell(25, 6, 'Broj Dosijea', 'LR', 0, 'C');
    $pdf->Cell(75, 6, 'DJELATNIK', 'LR', 0, 'C');
    $pdf->Cell(40, 6, 'JMBG', 'LR', 0, 'C');
    $pdf->Cell(75, 6, 'STATUS', 'LR', 0, 'C');
//    $pdf->Cell(40, 6, 'PREZIME', 'LR', 0, 'C');    
//    $pdf->Cell(44, 6, 'ZANIMANJE', 'LR', 0, 'C');
...
//    $urs = rs($row['DZRO'], $row['DPRO'], $row['SSTAZDANI'], $row['SSTAZMJ'], $row['SSTAZGOD']);
    ...
    $counter++;
    $pdf->Cell(25, 6, $counter, 'LRT', 0, 'C', 0, 0, 1);
    $pdf->Cell(70, 6, $row["IME"], 'RT', 0, 'C', 0, 0, 1);
    $pdf->Cell(70, 6, $row["PREZIME"], 'RT', 0, 'C', 0, 0, 1);
    $pdf->Cell(70, 6, $row["klk"], 'RT', 0, 'C', 0, 0, 1);
//    $pdf->Cell(70, 6, $row["TITULA"], 'RT', 0, 'C', 0, 0, 1);
    
    
    
//    $pdf->Cell(8, 6, $counter, 'LRT', 0, 'C', 0, 0, 1);
//    $pdf->Cell(25, 6, $row["BRD"], 'RT', 0, 'C');
//    $pdf->Cell(75, 6, $row["TITULA"] . ' ' . $row["IME"]. ' '. $row["PREZIME"], 'RT', 0, 'C', 0, 0, 1);
//    $pdf->Cell(40, 6, $row["JMBG"], 'RT', 0, 'C', 0, 0, 1);
//    $pdf->Cell(75, 6, $row["STATUS"], 'RT', 0, 'C', 0, 0, 1);
//    
//    
//    
//    
...
//    $pdf->Cell(44, 6, $row["ZANIMANJE"], 'RT', 0, 'C', 0, 0, 1);
