...
$q = "SELECT nomtit.IDe, IME, PREZIME, nomrm.NAZIV FROM djel "
        . "LEFT JOIN nomtit "
        . "ON djel.TITULA = nomtit.ID "
        . "INNER JOIN nomrm "
        . "ON nomrm.ID = djel.RM "
        . "WHERE STATUS LIKE '01%' AND PODVRSTA LIKE '06%' ORDER BY SSRM ASC ";

$r = mysqli_query($con, $q);
$counter = 1;
    $pdf->Cell(180, 5, 'Zdravstveni suradnici', 'LRT', 0, 'C', 0, 0, 1);
    $pdf->Ln(5);
    $pdf->Cell(5, 5, 'R.b.', 'LRT', 0, 'C', 0, 0, 1);
    $pdf->Cell(100, 5, 'Djelatnik', 'LRT', 0, 'C', 0, 0, 1);
    $pdf->Cell(75, 5, 'Radno mjesto', 'LRT', 0, 'C', 0, 0, 1);
    $pdf->Ln(5);
while($row = mysqli_fetch_assoc($r)){
    $pdf->Cell(5, 5, $counter++, 'LRT', 0, 'C', 0, 0, 1);
    $pdf->Cell(100, 5, $row['IDe'] .' '. $row['IME'] .' '. $row['PREZIME'], 'LRT', 0, 'C', 0, 0, 1);
    $pdf->Cell(75, 5, $row['NAZIV'], 'LRT', 0, 'C', 0, 0, 1);
    $pdf->Ln(5);
}
...
$q = "SELECT nomtit.IDe, IME, PREZIME, nomrm.NAZIV FROM djel "
        ...
        . "INNER JOIN nomrm "
        . "ON nomrm.ID = djel.RM "
        . "WHERE STATUS LIKE '01%' AND VRSTA = 4 AND SSRM = 15 AND PODVRSTA NOT LIKE '06%' ORDER BY SSRM ASC ";

$r = mysqli_query($con, $q);
$counter = 1;
    $pdf->Cell(180, 5, 'Nemedicinski djelatnici VSS', 'LRT', 0, 'C', 0, 0, 1);
    $pdf->Ln(5);
    $pdf->Cell(5, 5, 'R.b.', 'LRT', 0, 'C', 0, 0, 1);
    $pdf->Cell(100, 5, 'Djelatnik', 'LRT', 0, 'C', 0, 0, 1);
    $pdf->Cell(75, 5, 'Radno mjesto', 'LRT', 0, 'C', 0, 0, 1);
