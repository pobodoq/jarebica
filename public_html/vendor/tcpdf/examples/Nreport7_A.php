...
//            $pdf->Ln(5);
    $query = "SELECT BRD, IME, PREZIME, OTAC, JMBG, DZRO, DPRO, SSTAZDANI, SSTAZMJ, SSTAZGOD, DATUMR, nomtit.Ide as TITL, nomzan.NAZIV as ZANIMANJE, nomrm.NAZIV as RAM, nomskspr.NAZIV as SKASPR, nomss.NAZIV as SAS, nomstatusi.NAZIV as STATUS "
        . "FROM djel "
    ...
    while($row =  mysqli_fetch_assoc($clinicPeople)){
        ...
        $pdf->Cell(15, 5, $row["BRD"], 'RT', 0, 'C', 0, 0, 1);
        $pdf->Cell(80, 5, $row["TITL"] . ' ' . $row["IME"] . ' ' . $row["PREZIME"], 'RT', 0, 'C', 0, 0, 1);
        $pdf->Cell(36, 5, $row["JMBG"], 'RT', 0, 'C', 0, 0, 1);
        $pdf->Cell(40, 5, $row["STATUS"], 'RT', 0, 'C', 0, 0, 1);
        $pdf->Ln(5);  
    }

    $query = "SELECT * FROM odjel WHERE SIFRA LIKE '".$clinic['SIFRA']."%' ";
    ...
    while($division = mysqli_fetch_assoc($divisionList)){
        ...
        $query = "SELECT * FROM odsjek WHERE SIFRA LIKE '".$division['SIFRA']."%' ";
