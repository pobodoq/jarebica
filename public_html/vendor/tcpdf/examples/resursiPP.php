...
while($clinic = mysqli_fetch_assoc($clinicList)){
    ...
    if($checkNulls['kaunt']!=='0'){
        ...
        $pdf->SetHeaderData($logo, $logoWidth, $title, PHP_EOL . $clinic['NAZIV'] . PHP_EOL. date_format($date, 'd.m.Y'));
        $pdf->AddPage('L', 'A4');
        ...
        $query = "SELECT *, nomtit.IDe as TITL, nomrm.NAZIV as RAM, nomstatusi.NAZIV as STAT, nomss.NAZIV as SSRMJ "
                ...
                ."LEFT JOIN nomss "
                ."ON djel.SS = nomss.ID "
                ."WHERE djel.klinika LIKE '%".$clinic['SIFRA']."%' AND djel.ODJEL IS NULL ".$where." "
